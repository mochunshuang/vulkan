...
// NOTE: 小的，经常变换的数据才需要。一般是相机和鼠标和键盘输入有关的数据才有必要
      ...
      public:
        void createGraphicsPipeline(const VkSurfaceFormatKHR &swapChainSurfaceFormat)
        {
            ...
            VkShaderModule shaderModule =
                createShaderModule(readFile("shaders/push_constants.spv"));
    ...
    struct render
    {
      ...
      public:
        ...
        void recordCommandBuffer(vulkan::render_object &object, uint32_t imageIndex)
        {
            ...
            for (int i : {0, 1, 2})
            {
                PushConstantData push{.offset = {-0.5F + frame * 0.01, -0.4F + i * 0.25F},
