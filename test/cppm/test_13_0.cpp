...
class HelloTriangleApplication
{
  ...
  private:
    ...
    // 严格按要求：输入顶点数组 + 缩放比例 -> 输出同中心缩放后的顶点数组
    std::vector<Vertex> generateCenteredScaleVertices(
        const std::vector<Vertex> &sourceVertices, float scale, const glm::vec3 &newColor)
    {
    ...
    void mainLoop()
    {
        ...
        std::vector<Vertex> smallRectVertices =
            generateCenteredScaleVertices(bigRectVertices, // 输入：大矩形顶点
                                          0.5f, // 输入：缩放比例（0.5=缩小一半）
                                          glm::vec3(1.0f, 0.0f, 0.0f) // 新颜色（红色）
            );
        auto &smallRectIndices = bigRectIndices;
