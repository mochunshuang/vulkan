    ...
    */
    ...
    class Camera
    {
      public:
        Camera()
        {
            ...
            up = glm::vec3(0.0f, 0.0f, 1.0f); // Z轴向上
            worldUp = up;
            yaw = -135.0f;   // 看向(0,0,0)时从(2,2,2)位置的角度
            pitch = -35.26f; // 近似值
            movementSpeed = 2.5f;
        ...
        // 添加一个函数来直接设置视图矩阵（匹配原始代码）
        void setViewFromLookAt(const glm::vec3 &eye, const glm::vec3 &center,
    ...
    class GameObject
    {
      ...
      public:
        ...
        {
            // 为每个对象设置不同的位置
            static float offsets[3] = {-1.5f, 0.0f, 1.5f};

            UniformBufferObject ubo{};

            // 基础模型变换：不同位置
            ubo.model = glm::translate(glm::mat4(1.0f),
                                       glm::vec3(offsets[this->index], 0.0f, 0.0f));

            // 添加鼠标控制的旋转（左键拖动）
            ubo.model = glm::rotate(ubo.model, glm::radians(modelRotationY),
...

            {
                ...
                if ((presentation != nullptr) && (presentation->window != nullptr))
                {
                    ...
                    if (input.middleMousePressed)
                    {
                        ...
                        camera.position -= camera.right * input.dragDeltaX * panSpeed;
                        camera.position += camera.up * input.dragDeltaY * panSpeed;
...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void mainLoop()
    {
        ...
        const std::string MODEL_PATH = "models/viking_room.obj";     // 生产顶点和索引
        const std::string TEXTURE_PATH = "textures/viking_room.png"; // 生产纹理

        auto [vertices, indices] = loadModel(MODEL_PATH);
