...
{
    virtual void invoke_impl(void *args) noexcept = 0;
    virtual ~slot_interface() noexcept = default;
...
template <typename Rcvr, typename Impl, typename ArgsTuple>
struct slot_impl;

template <typename Rcvr, typename Impl, typename... Args>
struct slot_impl<Rcvr, Impl, std::tuple<Args...>> : slot_interface
{
    void invoke_impl(void *args) noexcept override
    {
...
struct connect_object
{
    struct connect_ptr
    {
        ...
        constexpr void release() noexcept
        {
            ...
            {
                delete slot_;
                // std::cout << "delete slot_\n";
                return;
            }
            delete this;
            // std::cout << "delete shared_slot_ptr\n";
        }
    ...
    {
        // NOTE: 多线程需要保持线程安全
        slot_interface *s = new (std::nothrow)
            slot_impl<Rcvr, slot_type, typename traits_slot<slot_type>::ref_args_tuple>{
                recr, std::move(slot)};
        if (s == nullptr)
            return nullptr;
...
void test_slot_ptr()
{
    ...
    }
    {
        ...
        int listen_key{-1};
        ret = connect_object::connect<my_signal::signal_click>(
            &signaler, &sloter, [&](my_slot *, int key) noexcept { listen_key = key; });
