...
struct sync_object
{
    ...
    sync_object() = default;
    sync_object(vk::raii::Device &device)
    {
...
struct glfw_context
{
    ...
    {
        sync_obj = sync_object(logicalDevice.device);
    }
    /*
...
*/
    void drawFrame()
    {
        auto &queue = logicalDevice.queue;
        auto &swapChain = swapchain.swapChain;
        auto &presentCompleteSemaphore = sync_obj.presentCompleteSemaphore;
        auto &device = logicalDevice.device;
        auto &drawFence = sync_obj.drawFence;

        auto &commandBuffer = swapchain.commandBuffer;
        auto &renderFinishedSemaphore = sync_obj.renderFinishedSemaphore;
        ...
        // NOTE: 3. 记录一个命令缓冲区
        swapchain.recordCommandBuffer(imageIndex, pipeline.graphicsPipeline);
...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void mainLoop()
    {
        ...
        ctx.logicalDevice.device.waitIdle(); // wait for device to finish operations
