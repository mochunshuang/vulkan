...
class HelloTriangleApplication
{
  ...
  private:
    ...
    VkRenderPass renderPass; // NOTE: 渲染过程
    ...
    void cleanup()
    {
        ...
        // NOTE: 就像管线布局一样，渲染通道将在整个程序中被引用，因此它应该只在最后清理。
    ...
    // NOTE: 在完成管线的创建之前，我们需要告诉 Vulkan 将在渲染时使用的帧缓冲附件。
    // 我们需要指定将有多少颜色和深度缓冲区，每个缓冲区使用多少个采样，以及它们的内容在整个渲染操作中应该如何处理。
    void createRenderPass()
    {
        // 在我们的例子中，我们将只有一个颜色缓冲区附件，由交换链中的一个图像表示
        VkAttachmentDescription colorAttachment{};
        colorAttachment.format = swapChainImageFormat;
        // 颜色附件的 format应该与交换链图像的格式匹配，而且我们还没有进行多重采样，
        // 所以我们将坚持使用 1 个采样。
        ...
        // 在我们的例子中，我们将使用清除操作在绘制新帧之前将帧缓冲区清除为黑色。
        colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
        // VK_ATTACHMENT_STORE_OP_STORE：渲染的内容将存储在内存中，并且可以稍后读取
        // VK_ATTACHMENT_STORE_OP_DONT_CARE：渲染操作后帧缓冲区的内容将未定义
        colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        // loadOp 和 storeOp 应用于颜色和深度数据，而 stencilLoadOp / stencilStoreOp
        // 应用于模板数据.
        // 我们的应用程序不会对模板缓冲区执行任何操作，因此加载和存储的结果无关紧要。
        ...
        // VK_IMAGE_LAYOUT_UNDEFINED表示不在乎图像之前的布局是什么
        colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        // 我们希望图像在渲染后可以使用交换链进行呈现
        colorAttachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;

        // NOTE: 子过程和附件引用
        // 单个渲染过程可以由多个子过程组成。子过程是后续渲染操作，依赖于前一个过程中的帧缓冲区内容，例如依次应用的一系列后处理效果。
        VkAttachmentReference colorAttachmentRef{};
        // attachment 参数通过附件描述数组中的索引指定要引用的附件。我们的数组由单个
        // VkAttachmentDescription 组成，因此其索引为 0
        colorAttachmentRef.attachment = 0;
        // 该布局将为我们提供最佳性能，正如其名称所暗示的那样。
        colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

        // NOTE: 子过程使用 VkSubpassDescription 结构描述
        VkSubpassDescription subpass{};
        // Vulkan 未来可能还支持计算子过程，因此我们必须明确说明这是一个图形子过程。
        ...
        */
        ...
        // NOTE: 渲染过程.
        // 现在已经描述了附件和引用它的基本子过程，我们可以创建渲染过程本身。
