...
class HelloTriangleApplication
{
  ...
  private:
    ...
    VkCommandPool commandPool; // NOTE: 命令池

    // 命令缓冲区会在其命令池被销毁时自动释放，因此我们不需要显式清理。
    VkCommandBuffer commandBuffer; // NOTE: 命令缓冲区
    ...
    void cleanup()
    {
        // NOTE:命令将在整个程序中使用以在屏幕上绘制内容，因此池应该只在末尾被销毁
    ...
    void createCommandPool()
    {
        ...
        // 命令缓冲区通过在设备队列（例如我们检索到的图形和呈现队列）之一上提交来执行。每个命令池只能分配在单一类型的队列上提交的命令缓冲区。
    ...
    // NOTE: 命令缓冲区记录: 该函数将我们要执行的命令写入命令缓冲区
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        // flags 参数指定我们将如何使用命令缓冲区
        ...
        // pInheritanceInfo参数仅与辅助命令缓冲区相关。它指定从调用主命令缓冲区继承哪个状态。
        beginInfo.pInheritanceInfo = nullptr; // Optional

        // 如果命令缓冲区已经被记录过一次，那么对 vkBeginCommandBuffer
        // 的调用将隐式重置它。不可能在稍后的时间向缓冲区追加命令
        ...
        // 我们需要为要绘制的交换链图像绑定帧缓冲。
        // 传入的 imageIndex 参数，我们可以为当前的交换链图像选择正确的帧缓冲
        renderPassInfo.framebuffer = swapChainFramebuffers[imageIndex];
        // 接下来的两个参数定义了渲染区域的大小。为了获得最佳性能，它应该与附件的大小相匹配。
        renderPassInfo.renderArea.offset = {0, 0};
        renderPassInfo.renderArea.extent = swapChainExtent;

        // 我已将清除颜色定义为简单的 100% 不透明度的黑色。
        // 最后两个参数定义了用于 VK_ATTACHMENT_LOAD_OP_CLEAR 的清除值
        ...
        // NOTE: 基本绘图命令: 现在可以绑定图形管线
        // 第二个参数指定管线对象是图形管线还是计算管线。
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          graphicsPipeline);

        // NOTE: 我们确实为此管线指定了视口和剪刀状态为动态。
...
/**
 * @brief 在 Vulkan
 * 中，像绘制操作和内存传输这样的命令不是通过函数调用直接执行的。
 * 你必须将所有要执行的操作记录在命令缓冲区对象中。这样做的好处是，当我们准备告诉
 * Vulkan 我们想要做什么时，所有命令都会一起提交，Vulkan可以更有效地处理命令，
 * 因为所有命令都是一起可用的。此外，如果需要，这允许在多个线程中进行命令记录。
 *
 * @return int
 */
int main()
