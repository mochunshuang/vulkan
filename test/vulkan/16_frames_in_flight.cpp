...
// NOTE: 1. 飞行帧的个数。在飞行中有两帧，CPU和GPU可以同时处理自己的任务。
// 在飞行中有三帧或更多帧时，CPU可以领先于GPU，增加帧延迟。
// 一般来说，不需要额外的延迟。但是让应用程序控制飞行中的帧数是Vulkan明确的另一个例子。
// 每个帧都应该有自己的命令缓冲区、信号量集和围栏。重命名，然后将它们更改为对象的std::vector
...
class HelloTriangleApplication
{
  ...
  private:
    ...
    // NOTE: 2. vector 化帧需要的上下文对象
    // 每个帧都应该有自己的命令缓冲区、信号量集和围栏
    ...
    uint32_t currentFrame = 0; // NOTE: 跟踪当前帧。飞行帧中同一时间只能有一个呈现
    ...
    // NOTE: 5. 修改drawFrame函数以使用正确的对象。 使用 currentFrame 索引
    void drawFrame()
    {
        ...
        /*
        我们现在已经实现了所有必要的同步，
        以确保排队的工作帧不超过MAX_FRAMES_IN_FLIGHT帧，并且这些帧不会相互重叠。
        请注意，代码的其他部分，如最终清理，可以依赖更粗糙的同步，如vkDeviceWaitIdle。
        NOTE: 此外，我们可以使用时间线信号量来代替这里介绍的二进制信号量。
        NOTE: 请注意，时间线信号量对于处理计算和图形队列特别有用
        */
    }
...
int main()
{
    try
    {
        ...
        // 解决这个问题的方法是允许多帧同时在飞行中，也就是说，允许一帧的渲染不干扰下一帧的录制。
        // NOTE: 我们如何做到这一点？ 复制，用内存空间存放未来要使用的资源
        // 渲染过程中访问和修改的任何资源都必须复制。因此，我们需要多个命令缓冲区、信号量和栅栏。
