...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void createInstance()
    {
        constexpr vk::ApplicationInfo appInfo{.pApplicationName = "Vulkan Profiles Demo",
                                              .applicationVersion =
    ...
    void checkFeatureSupport()
    {
        ...
        else
        {
            ...
            std::cout << "Falling back to traditional rendering (profile not supported)"
                      << std::endl;
    ...
    {
        auto vertShaderCode = readFile("shaders/27_shader_depth_vert.spv");
        auto fragShaderCode = readFile("shaders/27_shader_depth_frag.spv");
    ...
    void createFramebuffers()
    {
        ...
        {
            std::array<vk::ImageView, 3> attachments = {*colorImageView, *depthImageView,
    ...
    void recordCommandBuffer(uint32_t imageIndex)
    {
        ...
        commandBuffers[currentFrame].drawIndexed(static_cast<uint32_t>(indices.size()), 1,
                                                 0, 0, 0);
...
int main()
{
    /*
    //NOTE: 兼容性很好
    Vulkan配置文件提供了一种标准化的方式：
        1. 定义应用程序所需的一组功能、扩展和限制
        2. 自动检查与用户硬件的兼容性
        3. 无需手动特征检测和后备路径
        4. 显著减少样板代码
    它们提供了对低级Vulkan API的更高级别抽象，从而更容易：
        1. 瞄准特定的硬件功能
        2. 确保跨不同GPU的兼容性
        3. 始终如一地实施最佳实践
        4. 减少特征检测的样板代码
    无需手动检查每个功能和扩展并实现回退路径，您只需指定应用程序所需的配置文件。
    如果用户的硬件不符合要求，Vulkan配置文件库将处理兼容性检查并提供适当的错误消息。
    @see https://docs.vulkan.org/tutorial/latest/13_Vulkan_Profiles.html
