...
class VulkanApplication
{
  public:
    ...
    void run(android_app *app)
    {
        ...
        // Note: onInputEvent is no longer a member of android_app in the current NDK
        // version Input events are now handled differently
        ...
        {
            while (ALooper_pollOnce(androidAppState.initialized ? 0 : -1, nullptr,
    ...
    static void handleAppCommand(android_app *app, int32_t cmd)
    {
        ...
        case APP_CMD_INIT_WINDOW:
            if (app->window != nullptr)
            {
                ...
                // Instead, we need to access the VulkanApplication instance through a
                // global variable or another mechanism. For now, we'll just set the
                // initialized flag.
  ...
  private:
    ...
    void createSurface()
    {
        ...
        if (vkCreateAndroidSurfaceKHR(*instance, &createInfo, nullptr, &_surface) !=
            VK_SUCCESS)
        {
    ...
    void pickPhysicalDevice()
    {
        ...
        if (devIter != devices.end())
        {
            ...
            vp::ProfileDesc profileDesc = {profileProperties.name,
    ...
    void createTextureImage()
    {
        ...
        KTX_error_code result = ktxTexture_CreateFromNamedFile(
            TEXTURE_PATH.c_str(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &kTexture);
        ...
        {
            // For KTX1 files or if we can't determine the format, use a reasonable
            // default
            textureFormat = vk::Format::eR8G8B8A8Unorm;
        }
        ...
        createImage(
            texWidth, texHeight, textureFormat, vk::ImageTiling::eOptimal,
                ...
                for (size_t i = 0; i < indexCount; i++)
                {
                             ...
                             TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT)
                    {
                        index = *reinterpret_cast<const uint32_t *>(indexData +
                             ...
                             TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE)
                    {
                        index = *reinterpret_cast<const uint8_t *>(indexData +
                                                                   i * indexStride);
                    }
    ...
    void updateUniformBuffers()
    {
        ...
        for (auto &gameObject : gameObjects)
        {
            ...
            gameObject.rotation.y +=
                rotationSpeed *
                deltaTime; // Slow rotation around Y axis scaled by frame time
    ...
    std::vector<char> readFile(const std::string &filename)
    {
        ...
        AAsset *asset = AAssetManager_open(androidAppState.app->activity->assetManager,
