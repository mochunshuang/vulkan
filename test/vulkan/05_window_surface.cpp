...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void createLogicalDevice()
    {
        ...
        // NOTE: 要求是同一个队列族
        // NOTE: get the first index into queueFamilyProperties which supports both
        // NOTE: graphics and present
        uint32_t queueIndex = ~0;
        for (uint32_t qfpIndex = 0; qfpIndex < queueFamilyProperties.size(); qfpIndex++)
        {
            if ((queueFamilyProperties[qfpIndex].queueFlags &
                 vk::QueueFlagBits::eGraphics) && // NOTE: 查询 present 支持
                physicalDevice.getSurfaceSupportKHR(qfpIndex, *surface))
            {
                // NOTE: 要求更严格而已
                // NOTE: found a queue family that supports both graphics and present
                queueIndex = qfpIndex;
                break;
            }
        }
