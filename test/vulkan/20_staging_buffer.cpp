...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void createVertexBuffer()
    {
        ...
        // NOTE: VK_BUFFER_USAGE_TRANSFER_SRC_BIT：缓冲区可用作内存传输操作中的源。
        // NOTE: VK_BUFFER_USAGE_TRANSFER_DST_BIT：缓冲区可用作内存传输操作中的目的地
        ...
        // NOTE: 2. 使用暂存缓冲区
        vk::BufferCreateInfo stagingInfo{.size = sizeof(vertices[0]) * vertices.size(),
                                         // NOTE: 源
        ...
        // NOTE: 3. 将数据从stagingBuffer复制到vertexBuffer。
        // 我们必须通过指定stagingBuffer的传输源标志和vertexBuffer的传输目标标志以及顶点缓冲区使用标志来表明我们打算这样做
                    ...
                    vk::DeviceSize size)
    {
        ...
        commandCopyBuffer.begin(vk::CommandBufferBeginInfo{
            // 我们只使用一次命令缓冲区，然后等待从函数返回，直到复制操作完成执行。
            // 使用VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT告诉驱动程序我们的意图是很好的做法。
            .flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit});
        // NOTE: 使用vkCmdCopyBuffer命令传输缓冲区内容。
...
int main()
{
    try
    {
        // NOTE: 暂存缓冲区
        /*
        我们现在拥有的顶点缓冲区工作正常，但是允许我们从CPU访问它的内存类型可能不是显卡本身读取的最佳内存类型。
        最优化的内存具有VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT标志，并且通常不能被专用显卡上的CPU访问。
        */
        /*
        我们将创建两个顶点缓冲区。一个暂存缓冲区在CPU可访问的内存中，用于将数据从顶点数组上传到，'
        最后一个顶点缓冲区在设备本地内存中。
        然后，我们将使用[缓冲区复制命令]将数据从暂存缓冲区移动到实际的顶点缓冲区。
        */
        // NOTE: 传输队列。
        // 缓冲区复制命令需要支持传输操作的队列族，使用VK_QUEUE_TRANSFER_BIT表示。
        // 好消息是，任何具有VK_QUEUE_GRAPHICS_BIT或VK_QUEUE_COMPUTE_BIT功能的队列族已经隐式支持VK_QUEUE_TRANSFER_BIT操作。
        // 在这种情况下，实现不需要在queueFlags中显式列出它。

        // NOTE: 改进现在可能看不见，但它的顶点数据现在正在从高性能内存加载。
        // 当我们开始渲染更复杂的几何图形时，这将很重要

        /*
        NOTE: 自定义分配器应该是有状态的。可以更细粒度的控制
        应该注意的是，在现实世界的应用程序中，您不应该为每个单独的缓冲区实际调用vkAllocateMemory。
        同时内存分配的最大数量受到maxMemoryAllocationCount物理设备限制的限制，该限制可能低至4096，
        即使在像NVIDIA GTX 1080这样的高端硬件上也是如此。
        同时为大量对象分配内存的正确方法是创建一个自定义分配器，
        该分配器使用我们在许多函数中看到的offset参数在许多不同对象之间拆分单个分配。

        NOTE: 计划未来提供的VulkanMemoryAllocator
        */
