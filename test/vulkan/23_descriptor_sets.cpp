...
// NOTE: 6. 对齐要求: C++结构中的数据应该如何与着色器中的统一定义相匹配
// 这些陷阱是始终明确对齐的一个很好的理由。
...
/*
Vulkan期望结构中的数据以特定方式在内存中对齐，例如：

1. 标量必须由N对齐（给定32位浮点数=4个字节）。
2. 一个float2必须对齐2N（=8字节）
3. 一个float3或float4必须对齐4N（=16字节）
4. 嵌套结构必须通过其成员的基本对齐四舍五入到16的倍数来对齐。
5. 一个float4x4矩阵必须具有与float4相同的对齐方式。

//NOTE: float4x4 定义在*.slang 着色器中。 总之必须考虑内存对齐
*/
...
class HelloTriangleApplication
{
  ...
  private:
    ...
    void createGraphicsPipeline()
    {
        ...
        // NOTE: 5. 修正 rasterizer、
        // 问题是因为我们在投影矩阵中做了Y翻转，顶点现在是按逆时针顺序而不是顺时针顺序绘制的。这会导致背面剔除开始，并阻止任何几何图形被绘制。
        vk::PipelineRasterizationStateCreateInfo rasterizer{
            .depthClampEnable = vk::False,
            .rasterizerDiscardEnable = vk::False,
            .polygonMode = vk::PolygonMode::eFill,
            .cullMode = vk::CullModeFlagBits::eBack,
            .frontFace = vk::FrontFace::eCounterClockwise, // NOTE: 与21不同
    ...
    void recordCommandBuffer(uint32_t imageIndex)
    {
        ...
        // NOTE: 4. 使用描述符集. 将每一帧的正确描述符集绑定到着色器中
        // 与顶点和索引缓冲区不同，描述符集并不是图形管道所独有的。因此，我们需要指定是否要将描述符集绑定到图形或计算管道。
    ...
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        ubo.view = lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f),
                          glm::vec3(0.0f, 0.0f, 1.0f));
        // NOTE: 7. 矩形投影成正方形是正常的。你可以写出固定的宽高比：800/600 很接近的
        ubo.proj = glm::perspective(glm::radians(45.0f),
                                    static_cast<float>(swapChainExtent.width) /
                                        static_cast<float>(swapChainExtent.height),
                                    0.1f, 10.0f);
        // NOTE: 下面的不太自然
        //  ubo.proj = glm::perspective(glm::radians(45.0f), 2.0f, 0.1f, 10.0f);
...
int main()
{
    // NOTE: 矩形变成了正方形，因为投影矩阵现在修正了长宽比。
    // updateUniformBuffer负责屏幕大小调整，所以我们不需要重新创建recreateSwapChain中的描述符集。
    try
    {
        // NOTE: 将为每个VkBuffer资源创建一个描述符集，以将其绑定到统一缓冲区描述符
