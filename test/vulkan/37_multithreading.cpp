...
class ThreadSafeResourceManager
{
  ...
  public:
    ...
    void allocateCommandBuffers(vk::raii::Device &device, uint32_t threadCount,
    ...
    vk::raii::CommandBuffer &getCommandBuffer(uint32_t index)
    {
        ...
        if (index >= commandBuffers.size())
        {
            throw std::runtime_error(
                "Command buffer index out of range: " + std::to_string(index) +
...
class MultithreadedApplication
{
  ...
  private:
    ...
    void initWindow()
    {
        ...
        window =
            glfwCreateWindow(WIDTH, HEIGHT, "Vulkan Multithreading", nullptr, nullptr);
    ...
    {
        auto app = reinterpret_cast<MultithreadedApplication *>(
    ...
    void initThreads()
    {
        ...
        for (uint32_t i = 0; i < threadCount; i++)
        {
            ...
            particleGroups[i].count = (i == threadCount - 1)
                                          ...
                                          : particlesPerThread;
            log("Thread ", i, " will process particles ", particleGroups[i].startIndex,
                " to ", (particleGroups[i].startIndex + particleGroups[i].count - 1),
                " (count: ", particleGroups[i].count, ")");
        }

        for (uint32_t i = 0; i < threadCount; i++)
        {
            workerThreads.emplace_back(&MultithreadedApplication::workerThreadFunc, this,
                                       i);
        ...
        while (!shouldExit)
        {
            ...
            try
            {
                ...
                vk::raii::CommandBuffer *cmdBuffer =
                    &resourceManager.getCommandBuffer(threadIndex);
    ...
    void createShaderStorageBuffers()
    {
        ...
        for (auto &particle : particles)
        {
            ...
            // Use square root of random value to ensure uniform distribution across the
            // area This prevents clustering near the center (which causes the donut
            // effect)
    ...
    void createComputeDescriptorSets()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
        {
            ...
            vk::DescriptorBufferInfo storageBufferInfoLastFrame(
                shaderStorageBuffers[(i + MAX_FRAMES_IN_FLIGHT - 1) %
                                     MAX_FRAMES_IN_FLIGHT],
                0, sizeof(Particle) * PARTICLE_COUNT);
    ...
    void recordComputeCommandBuffer(vk::raii::CommandBuffer &cmdBuffer,
                                    uint32_t startIndex, uint32_t count)
    {
        ...
        cmdBuffer.pushConstants<PushConstants>(
            *computePipelineLayout, vk::ShaderStageFlagBits::eCompute, 0, pushConstants);
    ...
    void recordGraphicsCommandBuffer(uint32_t imageIndex)
    {
        ...
        graphicsCommandBuffers[currentFrame].bindPipeline(
            vk::PipelineBindPoint::eGraphics, *graphicsPipeline);
        graphicsCommandBuffers[currentFrame].setViewport(
            0, vk::Viewport(0.0f, 0.0f, static_cast<float>(swapChainExtent.width),
                            static_cast<float>(swapChainExtent.height), 0.0f, 1.0f));
        graphicsCommandBuffers[currentFrame].setScissor(
            0, vk::Rect2D(vk::Offset2D(0, 0), swapChainExtent));
        graphicsCommandBuffers[currentFrame].bindVertexBuffers(
...
异步资源加载：使用多线程异步加载资源。
    */
    ...
    void drawFrame()
    {
        ...
        // If the framebuffer was resized, rebuild the swap chain before acquiring a new
        // image
        ...
        std::array<vk::Semaphore, 2> waitSemaphores = {
            *timelineSemaphore, *imageAvailableSemaphores[currentFrame]};
