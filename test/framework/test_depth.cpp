...
int main()
{
    try
    {
            ...
            structure_chain<VkGraphicsPipelineCreateInfo, VkPipelineRenderingCreateInfo>
                pipelineCreateInfoChain = {
                    {.stageCount = 2,
                     ...
                     .pDepthStencilState = &depthStencil, // diff: 添加深度测试状态
        ...
        auto recreateSwapChain = [&]() {
            ...
            cleanupSwapChain();
            cleanupDepthResources(); // diff: 清理深度资源
            createSwapChain();
            createImageViews();
            createDepthResources(); // diff: 重新创建深度资源
            // 更新相机宽高比
        ...
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
            ...
            // diff: 转换深度图像布局 - 修复：从UNDEFINED到DEPTH_ATTACHMENT_OPTIMAL
            my_render::transition_image_layout(
            ...
            // diff: 不再将深度图像布局转换回UNDEFINED，而是保持为DEPTH_ATTACHMENT_OPTIMAL
            my_render::transition_image_layout(
                commandBuffer, image, VK_IMAGE_ASPECT_COLOR_BIT,
