...
// diff: start . 128的大小,就是崩溃,黑屏,不要走极限
// 修改 PushConstants 结构体，只包含顶点数据（索引通过索引缓冲区传递）
...
// 添加静态断言确保大小正确
// static_assert(sizeof(PushConstants) == (16 * 4 + 16 * 4), "PushConstants size
// mismatch"); static_assert(offsetof(PushConstants, positions) == 0, "positions offset
// wrong"); static_assert(offsetof(PushConstants, colors) == 64, "colors offset wrong");

constexpr auto VERT_SHADER_PATH = "shaders/test_push_constant3_vert.spv";
    ...
    try
    {
        ...
        auto createGraphicsPipeline = [&]() {
            mcs::vulkan::shader_module vertshader{
                logical_device_, VERT_SHADER_PATH}; // diff: 推送常量着色器
            ...
            VkPipelineRasterizationStateCreateInfo rasterizer = {
                ...
                .cullMode = VK_CULL_MODE_NONE, // diff: 关闭
        ...
        /*
对于 vkCmdDraw：gl_VertexIndex = 顶点索引（从 0 到 vertexCount-1）

对于 vkCmdDrawIndexed：gl_VertexIndex = 索引缓冲区中的值
*/
        const uint32_t indices[] = {0, 1, 2, 2, 3, 0}; // 正方形的索引
        ...
        auto createIndexBuffer = [&]() {
            ...
            VkMemoryRequirements memRequirements =
                logical_device_.getBufferMemoryRequirements(indexBuffer);
            ...
            void *data;
            logical_device_.mapMemory(indexBufferMemory, 0, sizeof(indices), 0, &data);
            memcpy(data, indices, sizeof(indices));
            logical_device_.unmapMemory(indexBufferMemory);
        };
        ...
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
            ...
            // diff: start. 注意是逆时针绘制的.从0 -4. 着色器只需要顶点数据,索引可自定义
            // 推送常量数据 - 正方形的顶点属性
            // 确保数据在命令缓冲区记录时有效
            PushConstants pushConstants = {
                .positions = {
                    glm::vec4(-0.5f, -0.5f, 0.0f, 1.0f), // 左下
                    glm::vec4(0.5f, -0.5f, 0.0f, 1.0f),  // 右下
                    glm::vec4(0.5f, 0.5f, 0.0f, 1.0f),   // 右上
                    glm::vec4(-0.5f, 0.5f, 0.0f, 1.0f)   // 左上
                }};
            ...
            // NOTE: 下面是黑屏的
            // vkCmdDraw(commandBuffer, 6, 0, 0, 0);
            // 检查推送常量大小
            std::cout << "PushConstants size: " << sizeof(PushConstants) << " bytes\n";
            std::cout << "Expected: 128 bytes (16 * 4 * 2)\n";
            // assert(sizeof(PushConstants) == 128);
            // 调试：打印推送常量数据
            std::cout << "Pushing constants:\n";
            for (int i = 0; i < 4; i++)
            {
                std::cout << "Pos[" << i << "]: (" << pushConstants.positions[i].x << ", "
                          << pushConstants.positions[i].y << ")\n";
                // std::cout << "Col[" << i << "]: (" << pushConstants.colors[i].r << ", "
                //           << pushConstants.colors[i].g << ", "
                //           << pushConstants.colors[i].b << ")\n";
            }
            // 在 recordCommandBuffer 中添加视口调试
            std::cout << "Viewport: width=" << viewport.width
                      << ", height=" << viewport.height << "\n";
            std::cout << "Scissor: width=" << scissor.extent.width
...
// NOTE: 三角形绘制代码 + 通过推送常量传递数据 + 通过索引绘制 而不是硬编码三角形