...
class QuaternionCamera
{
    /*
// 使用四元数相机系统：
// position: 相机在世界空间中的位置
// rotation: 相机的旋转（使用四元数，以角度表示）
// 默认设置：相机在 (0,0,-3)，看向 (0,0,0)，上方向为 (0,1,0)
// 这等价于 glm::lookAt(glm::vec3(0,0,-3), glm::vec3(0,0,0), glm::vec3(0,1,0))
*/
  public:
    // ==================== 世界空间中的相机参数 ====================
    // 使用右手坐标系
    glm::vec3 position = glm::vec3(0.0f, 0.0f, -3.0f);      // 相机在世界空间中的位置
    glm::quat rotation = glm::quat(1.0f, 0.0f, 0.0f, 0.0f); // 相机旋转（四元数）
    ...
    // ==================== 构造函数和初始化 ====================
    // 默认设置：看向原点，上方向为+Y
    // 这等价于 target = glm::vec3(0,0,0), up = glm::vec3(0,1,0)
    ...
    glm::mat4 getViewMatrix() const
    {
        ...
        // 注意：在相机变换中，我们先旋转再平移，所以顺序是 translationMatrix *
        // rotationMatrix
    ...
    {
        glm::quat rotationQuat = glm::angleAxis(glm::radians(angleDegrees), axis);
    ...
    // diff: 删除旧的visible()函数，因为它依赖target
    // 新的可见范围计算基于相机朝向
    ...
    // 在沿着视线方向的距离平面上的可见范围
    VisibleRange visibleInViewDirection(float distanceFromCamera) const
    {
        return visibleAtDistance(distanceFromCamera);
    }
...
int main()
{
    try
    {
        ...
        // diff: 删除旧的visible()函数调用，使用新的可见范围计算
        // 获取沿着视线方向的可见范围（距离为3，即原点到相机位置的距离）
        auto range = scene.getCamera().visibleInViewDirection(
            3.0f); // 在距离相机3单位的平面上的范围
        ...
        // obj1.transform.setPosition(glm::vec3(range.minX, 0.0f, 0.0f)); // 屏幕左边缘
        // obj1.transform.setPosition(glm::vec3(range.maxX, 0.0f, 0.0f)); // 屏幕右边缘
        // obj1.transform.setPosition(glm::vec3(0.0f, range.minY, 0.0f)); // 屏幕下边缘
        // obj1.transform.setPosition(glm::vec3(0.0f, range.maxY, 0.0f)); // 屏幕上边缘
        ...
        */
        ...
        // camera.lookAt(p5); // obj5 将处于屏幕中心 - 等价于 camera.target = p5;
        // camera.lookAt(p2); // obj2 将处于屏幕中心 - 等价于 camera.target = p2;

        // diff: 上方向（Up）- 现在通过lookAt方法的第二个参数设置
        // 上下左右 颠倒,不仅仅Y颠倒 - 等价于 camera.up = glm::vec3(0.0f,-1.0f, 0.0f);
        // camera.lookAt(camera.position + camera.getForward(),
        ...
        // 测试1：基本旋转 - 等价于改变相机方向
        // camera.rotateYaw(15.0f); // 绕世界Y轴旋转15度（水平旋转）
        // camera.rotatePitch(15.0f); // 绕相机X轴旋转15度（垂直旋转）
        // camera.rotateRoll(10.0f); // 绕相机前轴Z轴旋转10度（滚转）
        ...
        // 测试3：绕任意轴旋转
        // camera.rotate(15.0f, glm::vec3(1.0f, 1.0f, 0.0f)); // 绕(1,1,0)轴旋转15度

        // 测试4：使用lookAt实现特定旋转（等价于旧的target功能）
        // camera.lookAt(glm::vec3(2.0f, 1.0f, 0.0f)); // 看向(2,1,0)点

        // 测试6：绕物体旋转（轨道相机）
        // float orbitRadius = 5.0f;
        // float orbitAngle = glm::radians(45.0f);
        // camera.position = glm::vec3(sin(orbitAngle) * orbitRadius, 1.0f,
        //                             cos(orbitAngle) * orbitRadius - 3.0f);
        // camera.lookAt(glm::vec3(0.0f, 0.0f, 0.0f));

        // 测试7：第一人称相机控制模拟
        // 向右平移并看向右边
        // camera.moveRight(1.0f);
        // camera.rotateYaw(-30.0f); // 看向左边

        // camera.moveForward(1.0f); // 画面会变大
        // camera.moveUp(1.0f); //画面向上移动
        ...
        // 测试9：检查旋转后的方向
        std::cout << "\n=== 相机方向测试 ===" << std::endl;
        std::cout << "相机位置: (" << camera.position.x << ", " << camera.position.y
                  << ", " << camera.position.z << ")" << std::endl;
        std::cout << "前方向: (" << camera.getForward().x << ", " << camera.getForward().y
                  << ", " << camera.getForward().z << ")" << std::endl;
        std::cout << "右方向: (" << camera.getRight().x << ", " << camera.getRight().y
                  << ", " << camera.getRight().z << ")" << std::endl;
        std::cout << "上方向: (" << camera.getUp().x << ", " << camera.getUp().y << ", "
                  << camera.getUp().z << ")" << std::endl;

        // 测试10：验证与旧系统的等价性
        // 默认相机应该看向(0,0,0)，上方向为(0,1,0)
        glm::vec3 expectedForward =
            glm::normalize(glm::vec3(0.0f, 0.0f, 0.0f) - camera.position);
        glm::vec3 actualForward = camera.getForward();
        std::cout << "\n=== 等价性验证 ===" << std::endl;
        std::cout << "预期前方向: (" << expectedForward.x << ", " << expectedForward.y
                  << ", " << expectedForward.z << ")" << std::endl;
        std::cout << "实际前方向: (" << actualForward.x << ", " << actualForward.y << ", "
                  << actualForward.z << ")" << std::endl;
        std::cout << "方向误差: " << glm::length(expectedForward - actualForward)
                  << std::endl;
        ...
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
            ...
            // orbitAngle += 0.5f; // 每帧旋转0.5度
            // if (orbitAngle > 360.0f)
            //     orbitAngle -= 360.0f;
            // float radAngle = glm::radians(orbitAngle);
            // camera.position = glm::vec3(sin(radAngle) * orbitRadius, orbitHeight,
            //                             cos(radAngle) * orbitRadius - 3.0f);
            // camera.lookAt(glm::vec3(0.0f, 0.0f, 0.0f));

            // 测试C：相机摆动测试（上下摆动）
            // static float bounceAngle = 0.0f;
            // bounceAngle += 2.0f;
            // if (bounceAngle > 360.0f)
            //     bounceAngle -= 360.0f;
            // float bounceHeight = sin(glm::radians(bounceAngle)) * 0.5f;
            // camera.position.y = bounceHeight - 3.0f;
            // camera.lookAt(glm::vec3(0.0f, 0.0f, 0.0f));

            // 测试D：缩放测试（前后移动）
            // static float zoomPhase = 0.0f;
            // zoomPhase += 1.0f;
            // if (zoomPhase > 360.0f)
            //     zoomPhase -= 360.0f;
            // float zoomDistance = 3.0f + sin(glm::radians(zoomPhase)) * 2.0f;
            ...
            // {
            //     auto &obj = objects[i];
            //     static float objRotation = 0.0f;
            //     objRotation += 1.0f; // 每帧增加1度
            //     if (objRotation > 360.0f)
            //         objRotation -= 360.0f;
            //     // 每个物体以不同速度旋转
            //     float rotationSpeed = 1.0f + i * 0.5f;
            //     obj.transform.setRotation(
            //         glm::angleAxis(glm::radians(objRotation * rotationSpeed),
            //                        glm::vec3(0.0f, 1.0f, 0.0f)));
            // }
            ...
            // {
            //     auto &obj = objects[i];
            //     static float movePhase = 0.0f;
            //     movePhase += 2.0f; // 每帧增加2度
            //     if (movePhase > 360.0f)
            //         movePhase -= 360.0f;

            //     // 每个物体相位差45度
            //     float radPhase = glm::radians(movePhase + i * 45.0f);
            //     float moveRadius = 1.5f + i * 0.3f; // 每个物体移动半径不同

            //     // 圆形移动轨迹
            //     float x = sin(radPhase) * moveRadius;
            //     float y = cos(radPhase) * (moveRadius * 0.5f); // Y轴幅度小一些

            //     obj.transform.setPosition(glm::vec3(x, y, 0.0f));
            // }
            ...
            // {
            //     auto &obj = objects[i];
            //     static float scalePhase = 0.0f;
            //     scalePhase += 3.0f; // 每帧增加3度
            //     if (scalePhase > 360.0f)
            //         scalePhase -= 360.0f;

            //     float radPhase = glm::radians(scalePhase + i * 30.0f);
            //     float scale = 0.5f + sin(radPhase) * 0.3f; // 在0.2-0.8之间脉动

            //     obj.transform.setScale(glm::vec3(scale, scale, 1.0f));
            // }
            ...
            //     animTime += 1.0f; // 时间推进
            //     if (animTime > 360.0f) animTime -= 360.0f;
            //
            //     float radTime = glm::radians(animTime);
            //     float phaseOffset = i * 0.5f; // 每个物体相位偏移
            //
            //     // 1. 旋转：绕不同轴旋转
            //     glm::vec3 rotationAxis = glm::normalize(glm::vec3(
            //         sin(radTime * 0.3f + phaseOffset),
            //         cos(radTime * 0.5f + phaseOffset),
            //         sin(radTime * 0.7f + phaseOffset)
            //     ));
            //     float rotationAngle = animTime * (1.0f + i * 0.2f);
            //     obj.transform.setRotation(
            //         glm::angleAxis(glm::radians(rotationAngle), rotationAxis));
            //
            //     // 2. 平移：Lissajous曲线轨迹
            //     float x = sin(radTime * 1.5f + phaseOffset) * 2.0f;
            //     float y = cos(radTime * 2.0f + phaseOffset) * 1.5f;
            //     float z = sin(radTime * 1.0f + phaseOffset) * 0.5f;
            //     obj.transform.setPosition(glm::vec3(x, y, z));
            //
            //     // 3. 缩放：脉动效果
            //     float scale = 0.3f + sin(radTime * 3.0f + phaseOffset) * 0.2f;
            //     obj.transform.setScale(glm::vec3(scale, scale, scale));
            // }

            // 测试E5：简单水平摆动（适合看到屏幕边界）
            // for (size_t i = 0; i < objects.size(); i++)
            // {
            //     auto &obj = objects[i];
            //     static float swingTime = 0.0f;
            //     swingTime += 2.0f;
            //     if (swingTime > 360.0f)
            //         swingTime -= 360.0f;

            //     // 简单左右摆动
            //     float x = sin(glm::radians(swingTime + i * 90.0f)) * range.maxX * 0.8f;
            //     obj.transform.setPosition(glm::vec3(x, 0.0f, 0.0f));
            // }
            ...
            // {
            //     auto &obj = objects[i];
            //     static float inOutTime = 0.0f;
            //     inOutTime += 1.5f;
            //     if (inOutTime > 360.0f)
            //         inOutTime -= 360.0f;

            //     // 在屏幕内外移动
            //     float radTime = glm::radians(inOutTime + i * 120.0f);
            //     float x = sin(radTime) * (range.maxX * 1.5f); // 有时会超出屏幕
            //     float y = cos(radTime) * (range.maxY * 1.5f);
            ...
            // {
            //     auto &obj = objects[i];
            //     static float depthTime = 0.0f;
            //     depthTime += 1.0f;
            //     if (depthTime > 360.0f)
            //         depthTime -= 360.0f;

            //     // 在Z轴上前后移动（测试近大远小）
            //     float radTime = glm::radians(depthTime + i * 60.0f);
            //     float z = sin(radTime) * 2.0f; // 在-2到+2之间移动

            //     obj.transform.setPosition(glm::vec3(0.0f, 0.0f, z));
            // }
