...
namespace mcs::vulkan::camera
{
    ...
    class CameraController
    {
      public:
        ...
        // diff: 获取和设置模型旋转角度
        [[nodiscard]] glm::vec3 get_model_rotation() const noexcept
        {
      ...
      private:
        ...
        void handle_mouse(glfw_input &input)
        {
            ...
            // diff: 鼠标左键按下时旋转模型（绕X,Y,Z轴）
            if (left_mouse_pressed_ && glm::length(delta) > 0.0f)
            {
                ...
                {
                    // Ctrl + 左键：绕Z轴旋转
                    model_rotation_.z += delta.x * sensitivity;
                    std::println("[CameraController] Left mouse model rotation (Z-axis): "
                                 "delta({:.1f}, {:.1f}), roll={:.2f}",
                                 delta.x, delta.y, model_rotation_.z);
                }
                else
                {
                    // 普通左键：绕X和Y轴旋转
                    model_rotation_.x += delta.y * sensitivity; // X轴旋转（俯仰）
                    model_rotation_.y += delta.x * sensitivity; // Y轴旋转（偏航）

                    std::println("[CameraController] Left mouse model rotation: "
                                 "delta({:.1f}, {:.1f}), pitch={:.2f}, yaw={:.2f}",
                                 delta.x, delta.y, model_rotation_.x, model_rotation_.y);
                }
            }

            // diff: 鼠标右键按下时旋转相机（俯仰和偏航）
            if (right_mouse_pressed_ && glm::length(delta) > 0.0f)
            {
                float sensitivity = 0.1f;
            ...
            // diff: 鼠标中键按下时平移相机（沿着屏幕平面）
            if (middle_mouse_pressed_ && glm::length(delta) > 0.0f)
            {
      ...
      private:
        ...
        bool left_mouse_pressed_ = false;   // diff: 左键状态 - 用于模型旋转
        bool middle_mouse_pressed_ = false; // diff: 中键状态 - 用于相机平移
        bool right_mouse_pressed_ = false;  // diff: 右键状态 - 用于相机旋转
        glm::vec2 last_mouse_pos_ = glm::vec2(0.0f);

        // diff: 模型旋转角度 (X, Y, Z轴)
        glm::vec3 model_rotation_ = glm::vec3(0.0f);
    };
} // namespace mcs::vulkan::camera
    ...
    try
    {
        ...
        auto drawFrame = [&]() {
            ...
            if (total_time >= 1.0f)
            {
                ...
                auto model_rot = camera_controller->get_model_rotation();
                ...
                std::println("[Camera] Pos({:.2f}, {:.2f}, {:.2f}), Yaw/Pitch({:.1f}, "
                             ...
                             forward.z);
                std::println("[Model] Rotation: X={:.1f}, Y={:.1f}, Z={:.1f}",
                             model_rot.x, model_rot.y, model_rot.z);
            ...
            // diff: 更新Uniform Buffer - 使用相机控制器的模型旋转
            CameraUBO ubo{};
            auto model_rot = camera_controller->get_model_rotation();

            // 使用欧拉角创建模型旋转矩阵 (Y-X-Z顺序)
            ubo.model = glm::eulerAngleYXZ(glm::radians(model_rot.y), // Yaw (绕Y轴)
                                           glm::radians(model_rot.x), // Pitch (绕X轴)
                                           glm::radians(model_rot.z)  // Roll (绕Z轴)
            );
