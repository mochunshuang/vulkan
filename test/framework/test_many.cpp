...
constexpr auto VERT_SHADER_PATH = "shaders/test_many_vert.spv";
...
struct mesh_data
{
  ...
  private:
    ...
    void createVertexBufferForFrame(FrameBuffers &fb)
    {
        ...
        syncStatusForFrame(fb, vertices.data(), BUFFER_SIZE);
    }
...
// NOTE: 和 test_vertex_input.cpp 不同在于，用空间避免了  vkQueueWaitIdle(queue); 的等待
// NOTE: CPU内存应该够用。
int main()
{
    try
    {
        ...
        */
        ...
        auto createGraphicsPipeline = [&]() {
            ...
            // diff: [new] 修改管线布局以包含推送常量
            VkPipelineLayoutCreateInfo pipelineLayoutInfo = {
                .sType = sType<VkPipelineLayoutCreateInfo>(),
                .setLayoutCount = 0,
                .pushConstantRangeCount = 1,              // diff: [new] 启用推送常量
                .pPushConstantRanges = &pushConstantRange // diff: [new]
            };
        ...
        // diff: [new] 修改recordCommandBuffer以渲染多个对象并添加动画
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
                                       uint32_t currentFrame,
                                       const std::vector<std::unique_ptr<mesh_data>>
                                           &meshes, // 改为接收网格列表
                                       uint32_t imageIndex,
                                       float time) { // diff: [new] 添加时间参数用于动画
            ...
            for (size_t i = 0; i < meshes.size(); ++i)
            {
                ...
                {
                    // 添加基于时间的动画
                    float animX = std::sin(time) * 0.1f;         // 水平摆动
                    float animY = std::cos(time * 1.5f) * 0.08f; // 垂直摆动
                    baseOffsetX += animX;
                    baseOffsetY += animY;

                    // 添加旋转动画
                    float rotation = time * 0.8f;
                    pushConsts.model = glm::translate(
                        glm::mat4(1.0f), glm::vec3(baseOffsetX, baseOffsetY, 0.0f));
                    pushConsts.model = glm::rotate(pushConsts.model, rotation,
                                                   glm::vec3(0.0f, 0.0f, 1.0f));

                    // 添加脉动缩放
                    float pulse = 0.05f * std::sin(time * 2.0f);
                    float scale = 0.5f + pulse;
                    pushConsts.model =
                        glm::scale(pushConsts.model, glm::vec3(scale, scale, 1.0f));
                }
                else
                {
                    // 其他物体静止
                ...
                // 绑定并绘制这个物体
                meshes[i]->bind(commandBuffer, currentFrame);
        ...
        // diff: [new] 准备好多个对象的顶点和索引数据
        // 第一个对象：四边形（原代码中的）
        const std::vector<Vertex> vertices_square = {
            {{-0.5f, -0.5f}, {1.0f, 0.0f, 0.0f}}, // 红
            {{0.5f, -0.5f}, {0.0f, 1.0f, 0.0f}},  // 绿
            {{0.5f, 0.5f}, {0.0f, 0.0f, 1.0f}},   // 蓝
            {{-0.5f, 0.5f}, {1.0f, 1.0f, 1.0f}}   // 白
        };
        const std::vector<uint32_t> indices_square = {0, 1, 2, 2, 3, 0};

        // 第二个对象：三角形
        const std::vector<Vertex> vertices_triangle = {
            {{-0.3f, -0.3f}, {1.0f, 0.5f, 0.0f}}, // 橙色
            {{0.3f, -0.3f}, {0.5f, 0.0f, 1.0f}},  // 紫色
            {{0.0f, 0.3f}, {0.0f, 1.0f, 0.5f}}    // 青色
        };
        const std::vector<uint32_t> indices_triangle = {0, 1, 2};

        // 第三个对象：另一个三角形（倒置）
        const std::vector<Vertex> vertices_triangle2 = {
            {{-0.3f, -0.3f}, {1.0f, 0.0f, 1.0f}}, // 洋红
            {{0.3f, -0.3f}, {0.0f, 1.0f, 1.0f}},  // 青色
            {{0.0f, 0.3f}, {1.0f, 1.0f, 0.0f}}    // 黄色
        };
        const std::vector<uint32_t> indices_triangle2 = {0, 1, 2};

        // diff: [new] 创建多个mesh对象
        std::vector<std::unique_ptr<mesh_data>> meshes;
        meshes.push_back(std::make_unique<mesh_data>(physicalDevice, logical_device_,
                                                     graphicsQueue, commandPool,
                                                     vertices_square, indices_square));
        meshes.push_back(std::make_unique<mesh_data>(
            physicalDevice, logical_device_, graphicsQueue, commandPool,
            vertices_triangle, indices_triangle));
        meshes.push_back(std::make_unique<mesh_data>(
            physicalDevice, logical_device_, graphicsQueue, commandPool,
            vertices_triangle2, indices_triangle2));
        ...
        auto drawFrame = [&]() {
            ...
            // diff: [new] 录制命令缓冲区，传入所有网格对象和时间
            recordCommandBuffer(commandBuffer, currentFrame, meshes, imageIndex, time);
