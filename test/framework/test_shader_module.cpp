...
int main()
{
    ...
    try
    {
        ...
        {
            mcs::vulkan::shader_module shader{ctx.ref_logical_device(),
                                              "shaders/09_shader_base.spv"};
            mcs::MCS_ASSERT(shader.valid());
            auto d = std::move(shader);
            mcs::MCS_ASSERT(not shader.valid());
        }
