...
// diff: 新增MSAA相关的全局变量和辅助函数
namespace
{
    // diff: 获取最大可用采样计数
    VkSampleCountFlagBits getMaxUsableSampleCount(physical_device &physicalDevice)
    {
    ...
    // diff: 创建MSAA颜色图像
    void createColorResources(physical_device &physicalDevice, logical_device &device,
                              VkFormat swapChainImageFormat, VkExtent2D swapChainExtent,
                              VkSampleCountFlagBits msaaSamples, VkImage &colorImage,
                              VkDeviceMemory &colorImageMemory,
                              VkImageView &colorImageView)
    {
        VkImageCreateInfo imageInfo = {.sType = sType<VkImageCreateInfo>(),
        ...
        colorImage = device.createImage(imageInfo);

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(device.raw_data(), colorImage, &memRequirements);
        ...
        colorImageMemory = device.allocateMemory(allocInfo);
        vkBindImageMemory(device.raw_data(), colorImage, colorImageMemory, 0);
        ...
        colorImageView = device.createImageView(viewInfo);
    }
} // namespace

// NOTE: 在 test_depth.cpp 之上添加 msaa.
int main()
{
    try
    {
        ...
        std::println("[MSAA] Using sample count: {}", static_cast<int>(msaaSamples));
        ...
        auto createColorResources = [&]() {
            ::createColorResources(physicalDevice, logical_device_, swapChainImageFormat,
                                   swapChainExtent, msaaSamples, colorImage,
                                   colorImageMemory, colorImageView);
        };

        // diff: 清理MSAA颜色资源的函数
        auto cleanupColorResources = [&]() {
            if (colorImageView != VK_NULL_HANDLE)
            {
                logical_device_.destroyImageView(colorImageView);
                colorImageView = VK_NULL_HANDLE;
            }
            if (colorImage != VK_NULL_HANDLE)
            {
                logical_device_.destroyImage(colorImage);
                colorImage = VK_NULL_HANDLE;
            }
            if (colorImageMemory != VK_NULL_HANDLE)
            {
                logical_device_.freeMemory(colorImageMemory);
        ...
        auto createDepthResources = [&]() {
            ...
            VkImageCreateInfo imageInfo = {
                ...
                .mipLevels = 1,
                .arrayLayers = 1,
                .samples = msaaSamples, // diff: 使用MSAA采样计数
                .tiling = VK_IMAGE_TILING_OPTIMAL,
        ...
        // c0:  test_camera2.cpp 创建Uniform Buffer Manager
            ...
            structure_chain<VkGraphicsPipelineCreateInfo, VkPipelineRenderingCreateInfo>
                pipelineCreateInfoChain = {
                    {.stageCount = 2,
                     ...
                     .pMultisampleState = &multisampling, // diff: 使用MSAA状态
                     .pDepthStencilState =
        ...
        auto recreateSwapChain = [&]() {
            ...
            cleanupColorResources(); // diff: 清理颜色资源
            createSwapChain();
            createImageViews();
            createColorResources(); // diff: 重新创建颜色资源
        ...
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
            ...
            my_render::transition_image_layout(
                commandBuffer, colorImage, VK_IMAGE_ASPECT_COLOR_BIT,
                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, {},
                VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);

            // diff: 转换交换链图像布局到颜色附件最佳
            my_render::transition_image_layout(
            ...
            VkRenderingAttachmentInfo colorAttachment = {
                ...
                .resolveImageLayout = VkImageLayout::
                    VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // diff: 解析图像布局
        ...
        auto drawFrame = [&]() {
            ...
            if (total_time >= 1.0f)
            {
                ...
                std::println("[MSAA] Sample count: {}",
                             static_cast<int>(msaaSamples)); // diff: 显示MSAA信息
            ...
            // c0:  test_camera2.cpp 更新Uniform Buffer - 使用四元数创建模型矩阵
            CameraUBO ubo{};
            auto model_rotation = camera_controller->get_model_rotation();
        ...
        // 主循环
        std::println("\n=== Camera Test with MSAA ===");
        std::println("MSAA Sample Count: {}", static_cast<int>(msaaSamples));
        ...
        std::println("Note: Depth testing is now enabled for proper 3D rendering.\n");
        std::println("Note: MSAA is enabled for anti-aliasing.\n");
