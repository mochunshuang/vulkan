...
// NOTE: 生成绘制索引数据?
constexpr auto VERT_SHADER_PATH = "shaders/test_backend_drawIndexed_vert.spv";
...
try
{
    ...
    std::vector<uint32_t> indices = {0, 1, 2, 3, 4,  5, // NOLINT // NOLINTNEXTLINE
                                     6, 7, 8, 9, 10, 11, 12, 13, 14, 14, 15, 16};
    // std::vector<uint32_t> indices = {
    //     3,
    //     4,
    //     5, // NOLINT // NOLINTNEXTLINE
    // };
    // 修改索引值,绘制不同的三角形,说明确实如此
    // std::vector<uint32_t> indices = {0, 1, 2};
    const auto BUFFER_SIZE = sizeof(indices[0]) * indices.size();

    auto indexBuffer = mcs::vulkan::tools::create_buffer(
        ctx.logicalDevice(),
    ...
    auto stagingBuffer =
        mcs::vulkan::tools::staging_buffer(ctx.logicalDevice(), BUFFER_SIZE);
    stagingBuffer.copyDataToBuffer(indices.data(), BUFFER_SIZE);
    mcs::vulkan::tools::simple_copy_buffer(commandPool, graphicsAndPresentQueue,
                                           stagingBuffer.buffer(), indexBuffer.buffer(),
                                           {VkBufferCopy{.size = BUFFER_SIZE}});

    // indexBuffer = mcs::vulkan::tools::simple_index_buffer(
    //     commandPool, graphicsAndPresentQueue, indices.data(), BUFFER_SIZE);
