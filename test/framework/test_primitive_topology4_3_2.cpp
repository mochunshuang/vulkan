...
constexpr auto TITLE = "Geometry Demo with Multiple Shapes";
...
{
    glm::mat4 mvp;               // 模型-视图-投影矩阵
    glm::vec4 wireColorOverride; // 线框颜色覆盖
    int renderMode = 0;          // 渲染模式
    float lineWidth = 2.0f;      // 线宽
    float wireAlpha = 1.0f;      // 线框透明度
...
struct Material
{
    glm::vec3 fillColor = glm::vec3(0.5f, 0.5f, 0.8f);               // 默认蓝色填充
...
namespace GeometryGenerators
{
    ...
    // 球体几何体（修正索引）
    static std::unique_ptr<Geometry> createSphereGeometry(float radius = 0.5f,
                                                          ...
                                                          int heightSegments = 16)
    {
        ...
        for (int y = 0; y <= heightSegments; ++y)
        {
            for (int x = 0; x <= widthSegments; ++x)
            {
                ...
                glm::vec3 pos;
                pos.x = radius * cos(theta) * sinPhi;
                pos.y = radius * cos(phi);
                pos.z = radius * sin(theta) * sinPhi;

                geom->vertices.push_back(
                    {pos, geom->material.fillColor, geom->material.wireColor, 1.0f});
            }
        }
        ...
        for (int y = 0; y < heightSegments; ++y)
        {
            for (int x = 0; x < widthSegments; ++x)
            {
                ...
                // 第一个三角形
                geom->triangleIndices.push_back(a);
                geom->triangleIndices.push_back(c);
                geom->triangleIndices.push_back(b);

                // 第二个三角形
                geom->triangleIndices.push_back(b);
                geom->triangleIndices.push_back(c);
                geom->triangleIndices.push_back(d);
            }
        }
        ...
        for (int x = 0; x <= widthSegments; ++x)
        {
            ...
            {
                int a = (y * (widthSegments + 1)) + x;
                int b = ((y + 1) * (widthSegments + 1)) + x;
                geom->lineIndices.push_back(a);
                geom->lineIndices.push_back(b);
            }
        }
    ...
    // 圆柱体几何体（修正版）
    static std::unique_ptr<Geometry> createCylinderGeometry(float radiusTop = 0.5f,
                                                            ...
                                                            int heightSegments = 4)
    {
        ...
        {
            float v = static_cast<float>(y) / heightSegments;
            float currentRadius = radiusTop * (1.0f - v) + radiusBottom * v;
        ...
        // 添加顶部中心顶点
        int topCenterIndex = static_cast<int>(geom->vertices.size());
        geom->vertices.push_back({glm::vec3(0.0f, height * 0.5f, 0.0f),
                                  geom->material.fillColor, geom->material.wireColor,
                                  1.0f});

        // 添加底部中心顶点
        int bottomCenterIndex = static_cast<int>(geom->vertices.size());
        geom->vertices.push_back({glm::vec3(0.0f, -height * 0.5f, 0.0f),
                                  geom->material.fillColor, geom->material.wireColor,
                                  1.0f});
        ...
        for (int x = 0; x < radialSegments; ++x)
        {
            int a = topCenterIndex;
            int b = x;
            int c = (x + 1) % (radialSegments + 1);
            if (c == 0)
                c = radialSegments;
        ...
        // 生成底部圆盘三角形
        int bottomStart = heightSegments * (radialSegments + 1);
        for (int x = 0; x < radialSegments; ++x)
        {
            int a = bottomCenterIndex;
            int b = bottomStart + ((x + 1) % (radialSegments + 1));
            if (b == bottomStart + radialSegments + 1)
                b = bottomStart + radialSegments;
            int c = bottomStart + x;
    ...
    // 圆锥体几何体（使用圆柱体函数，顶部半径为0）
    static std::unique_ptr<Geometry> createConeGeometry(float radius = 0.5f,
                                                        float height = 1.0f,
                                                        int radialSegments = 16)
    {
        return createCylinderGeometry(0.0f, radius, height, radialSegments, 1);
    }

    // 平面几何体（修正版）
    static std::unique_ptr<Geometry> createPlaneGeometry(float width = 1.0f,
                                                         float height = 1.0f,
                                                         int widthSegments = 4,
                                                         int heightSegments = 4)
    {
    ...
    // 圆环几何体（修正版）
    static std::unique_ptr<Geometry> createTorusGeometry(float radius = 0.5f,
                                                         ...
                                                         int tubularSegments = 32)
    {
        ...
        {
            float u = static_cast<float>(i) / radialSegments * 2.0f * glm::pi<float>();
            for (int j = 0; j <= tubularSegments; ++j)
            {
                float v =
                    static_cast<float>(j) / tubularSegments * 2.0f * glm::pi<float>();

                glm::vec3 pos;
                pos.x = (radius + tubeRadius * cos(v)) * cos(u);
                pos.y = (radius + tubeRadius * cos(v)) * sin(u);
    ...
    // 胶囊体几何体（修正版）
    static std::unique_ptr<Geometry> createCapsuleGeometry(float radius = 0.3f,
                                                           float height = 1.0f,
                                                           int radialSegments = 16,
                                                           int heightSegments = 8)
    {
        ...
        // 生成顶点
        int totalSegments = heightSegments + 2; // 添加两个半球顶点的额外段
        for (int i = 0; i <= totalSegments; ++i)
        {
            float v = static_cast<float>(i) / totalSegments;
            float y, currentRadius;

            if (i <= heightSegments / 4)
            {
                // 上半球
                float phi = (0.5f - v * 2.0f) * glm::pi<float>();
                y = halfHeight + radius * sin(phi);
                currentRadius = radius * cos(phi);
            }
            else if (i >= totalSegments - heightSegments / 4)
            {
                // 下半球
                float phi = (v * 2.0f - 1.5f) * glm::pi<float>();
                y = -halfHeight + radius * sin(phi);
                currentRadius = radius * cos(phi);
            }
            else
            {
                // 圆柱部分
                y = halfHeight - (v - 0.25f) * 2.0f * halfHeight;
...
int main()
{
    try
    {
        ...
        GeometryType currentGeometryType = BOX;
        auto lastGeometryChange = std::chrono::steady_clock::now();

        // 创建立方体几何体
        auto createBoxGeometry = []() -> std::unique_ptr<Geometry> {
            ...
            box->vertices = {
                ...
                {{-0.5f, -0.5f, 0.5f},
                 ...
                 glm::vec3(1.0f),
                 1.0f},
                {{0.5f, -0.5f, 0.5f}, glm::vec3(0.8f, 0.2f, 0.2f), glm::vec3(1.0f), 1.0f},
                {{0.5f, 0.5f, 0.5f}, glm::vec3(0.8f, 0.2f, 0.2f), glm::vec3(1.0f), 1.0f},
                {{-0.5f, 0.5f, 0.5f}, glm::vec3(0.8f, 0.2f, 0.2f), glm::vec3(1.0f), 1.0f},
                // 后平面
                {{-0.5f, -0.5f, -0.5f},
            ...
            box->material.fillColor = glm::vec3(0.8f, 0.2f, 0.2f); // 红色
            box->material.wireColor = glm::vec3(1.0f, 1.0f, 1.0f); // 白色
            box->applyMaterial();
            return box;
        };

        mesh.setGeometry(createBoxGeometry());
        ...
        while (window.shouldClose() == 0)
        {
            ...
            if (now - lastModeChange > std::chrono::seconds(3))
            {
                lastModeChange = now;
                ...
                // 打印当前模式
                const char *modeNames[] = {"Fill Only", "Wire Only", "Both"};
                std::println("Display Mode: {}", modeNames[currentMode]);
            }

            // 每10秒切换一次几何体
            if (now - lastGeometryChange > std::chrono::seconds(10))
            {
                lastGeometryChange = now;

                currentGeometryType =
                    static_cast<GeometryType>((currentGeometryType + 1) % GEOMETRY_COUNT);

                // 根据类型创建新几何体
                std::unique_ptr<Geometry> newGeometry;
                glm::vec3 color;
                ...
                {
                case BOX:
                    newGeometry = createBoxGeometry();
                    color = glm::vec3(0.8f, 0.2f, 0.2f); // 红色
                    std::println("切换到: Box Geometry");
                    break;
                case SPHERE:
                    newGeometry = GeometryGenerators::createSphereGeometry(0.5f, 16, 16);
                    color = glm::vec3(0.2f, 0.8f, 0.2f); // 绿色
                    std::println("切换到: Sphere Geometry");
                    break;
                case CYLINDER:
                    newGeometry = GeometryGenerators::createCylinderGeometry(0.4f, 0.4f,
                                                                             1.0f, 16, 4);
                    color = glm::vec3(0.2f, 0.2f, 0.8f); // 蓝色
                    std::println("切换到: Cylinder Geometry");
                    break;
                case CONE:
                    newGeometry = GeometryGenerators::createConeGeometry(0.5f, 1.0f, 16);
                    color = glm::vec3(0.8f, 0.8f, 0.2f); // 黄色
                    std::println("切换到: Cone Geometry");
                    break;
                case PLANE:
                    newGeometry =
                        GeometryGenerators::createPlaneGeometry(1.0f, 1.0f, 4, 4);
                    color = glm::vec3(0.8f, 0.2f, 0.8f); // 紫色
                    std::println("切换到: Plane Geometry");
                    break;
                case TORUS:
                    newGeometry =
                        GeometryGenerators::createTorusGeometry(0.5f, 0.2f, 16, 32);
                    color = glm::vec3(0.2f, 0.8f, 0.8f); // 青色
                    std::println("切换到: Torus Geometry");
                    break;
                case CAPSULE:
                    newGeometry =
                        GeometryGenerators::createCapsuleGeometry(0.3f, 1.0f, 16, 8);
                    color = glm::vec3(0.8f, 0.5f, 0.2f); // 橙色
                    std::println("切换到: Capsule Geometry");
                    break;
                }
                ...
                if (newGeometry)
                {
                    ...
                    Material mat;
                    mat.fillColor = color;
                    mat.wireColor = glm::vec3(1.0f, 1.0f, 1.0f);
                    mat.wireColorOverride = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
                    mat.showFill = (currentMode != WIRE_ONLY);
                    mat.showWire = (currentMode != FILL_ONLY);
                    mat.fillAlpha = (currentMode == BOTH) ? 0.7f : 1.0f;
                    mat.wireAlpha = 1.0f;
                    mat.lineWidth = (currentMode == WIRE_ONLY) ? 3.0f : 2.0f;

                    newGeometry->setMaterial(mat);
                    mesh.setGeometry(std::move(newGeometry));
                }
            }
