...
// diff: [new] 修改mesh_data以使用设备地址和单缓冲
struct mesh_data
{
    // NOLINTBEGIN
          ...
          indices{std::move(indices)}
    {
        ...
        if (::vkCreateFence(device.raw_data(), &fenceInfo, nullptr, &updateFence) !=
            VK_SUCCESS)
            throw std::runtime_error("failed to create update fence!");
    ...
    ~mesh_data()
    {
        if (updateFence != VK_NULL_HANDLE)
            ::vkDestroyFence(device.raw_data(), updateFence, nullptr);
    }
    ...
    void setupBuffers()
    {
        ...
        createIndexBuffer();
        // diff: [new] 获取设备地址
        getBufferDeviceAddresses();
    }

    // diff: [new] 获取缓冲区的设备地址
    void getBufferDeviceAddresses()
    {
    ...
    // 安全地应用更新
    void applyQueuedUpdate(VkFence waitFence = VK_NULL_HANDLE)
    {
        if (!needUpdate)
            return;
        ...
        {
            // 等待上一帧完成（如果提供了fence）
            ::vkWaitForFences(device.raw_data(), 1, &waitFence, VK_TRUE, UINT64_MAX);
        }

        // 等待所有GPU操作完成，确保缓冲区不再被使用
        ::vkDeviceWaitIdle(device.raw_data()); // NOTE: 飞行帧 还是有必要的
        ...
        // 销毁旧缓冲区
        if (vertexBuffer.buffer() != VK_NULL_HANDLE)
            vertexBuffer = mcs::vulkan::buffer_base();
        if (indexBuffer.buffer() != VK_NULL_HANDLE)
            indexBuffer = mcs::vulkan::buffer_base();
  ...
  private:
    ...
    void createVertexBuffer()
    {
        ...
        if (::vkCreateBuffer(device.raw_data(), &bufferInfo, nullptr,
                             &vertexBufferHandle) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create vertex buffer!");
        }

        VkMemoryRequirements memRequirements;
        ::vkGetBufferMemoryRequirements(device.raw_data(), vertexBufferHandle,
                                        &memRequirements);
        ...
        {
            ::vkDestroyBuffer(device.raw_data(), vertexBufferHandle, nullptr);
            throw std::runtime_error("failed to allocate vertex buffer memory!");
        }

        if (::vkBindBufferMemory(device.raw_data(), vertexBufferHandle,
                                 vertexBufferMemory, 0) != VK_SUCCESS)
        {
        ...
        mcs::vulkan::copy_buffer(device, queue, commandPool, stagingBuffer.buffer(),
                                 vertexBufferHandle, BUFFER_SIZE);

        // diff: [new] 将创建的缓冲区包装到buffer_base中
        vertexBuffer =
            mcs::vulkan::buffer_base(device, vertexBufferHandle, vertexBufferMemory);
    }
    ...
    void createIndexBuffer()
    {
        ...
        if (::vkCreateBuffer(device.raw_data(), &bufferInfo, nullptr,
                             &indexBufferHandle) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create index buffer!");
        }

        VkMemoryRequirements memRequirements;
        ::vkGetBufferMemoryRequirements(device.raw_data(), indexBufferHandle,
                                        &memRequirements);
        ...
        {
            ::vkDestroyBuffer(device.raw_data(), indexBufferHandle, nullptr);
            throw std::runtime_error("failed to allocate index buffer memory!");
        }

        if (::vkBindBufferMemory(device.raw_data(), indexBufferHandle, indexBufferMemory,
                                 0) != VK_SUCCESS)
        {
        ...
        mcs::vulkan::copy_buffer(device, queue, commandPool, stagingBuffer.buffer(),
                                 indexBufferHandle, BUFFER_SIZE);

        // diff: [new] 将创建的缓冲区包装到buffer_base中
        indexBuffer =
            mcs::vulkan::buffer_base(device, indexBufferHandle, indexBufferMemory);
    }
  ...
  public:
    ...
    void bindIndexBuffer(VkCommandBuffer commandBuffer) const noexcept
    {
    ...
    try
    {
        ...
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer,
            ...
            PushConstants pushConstants = {
                .vertexBufferAddress = input_mesh.getVertexBufferAddress(),
                .indexBufferAddress = input_mesh.getIndexBufferAddress()};
            ...
            input_mesh.bindIndexBuffer(commandBuffer);
        ...
        auto drawFrame = [&]() {
            ...
            // 等待当前帧的fence，确保GPU完成对缓冲区的使用
            while (::vkWaitForFences(device, 1, &inFlightFences[currentFrame], VK_TRUE,
                                     UINT64_MAX) == VK_TIMEOUT)
                ;

            // 在等待fence后安全地应用更新
            // 使用当前帧的fence确保GPU已经完成对该帧的所有操作
            input_mesh.applyQueuedUpdate(inFlightFences[currentFrame]);
